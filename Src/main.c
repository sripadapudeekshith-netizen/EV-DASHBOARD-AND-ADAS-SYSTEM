...
int main(void)
{
	  ...
	  HAL_TIM_Base_Start_IT(&htim2);       /* count -1us   */
	  /* start interupt on receving the data
	   *
	   */
	    HAL_UART_Receive_IT(&huart1, &rx_byte ,1 );

	  EV_Init(&ev);//ev intialisatin
	  HCSR04_Init();//ultrasonic intialisation
	  ADAS_Init(&adas);//intialising adas
	  ...
	  while (1)
	  {
	      ...
	      if(sensor_flag == 1) //100 ms
	      {
	    	  sensor_flag = 0;
	    	  HCSR04_ReadAll();
	    	 //update ttc alarms , blind spot detection
	    	  ADAS_Update(&adas, &ev);
	    	  int ttcs =(int)adas.ttc_sec;
	    	  //convert to int and print
	    	  int ttcd =(int)(ttc_sec*10.0f)%10;
	    	  int frnt =(int)adas.front_cm;
	    	  int left =(int)adas.left_cm;
			   int right=(int)adas.right_cm;

			   char msg[60];
			   sprintf(msg ," F:%d L:%d  TTC:%ds COL:%d BSD:%d%d  ALM:%d  \r\n", frnt ,left, right );
			   HAL_UART_Transmit(&huart, (uint8_t*), streln(msg),100);
	    	  //validating motor temp ,soc and collision
...
/* UART RX byte -push the cell right buffer ,re -arm*/
